...
def get_dataset(dset, root, split, transform):
    return dset(root, split, transform=transform, download=True)
...
def prepare_data(dset, data_dir, batch_size, image_size, normalisation, shuffle=False, seed=10):
    if normalisation:
    ...
    train_loader, val_loader, trainval_loader = get_train_valid_loader(dset, data_dir, normalise_dict,
                                                batch_size, image_size, seed, shuffle=shuffle)

    test_loader = get_test_loader(dset, data_dir, normalise_dict, batch_size, image_size, shuffle=shuffle)

    all_loader = get_train_test_loader(dset, data_dir, normalise_dict, batch_size, image_size, shuffle=shuffle )
