...
from rank_correlation import (load_score, w_kendall_metric)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-me', '--method', type=str, default='disco',
    ...
    score_path = f'metrics/group1/{metric}/{dset}_metrics.json'
    score, _ = load_score(score_path)

    disco_score = {}
    for model in score.keys():
        ncc_score = score[model]['ncc']
        ratio_score = score[model]['ratio']

        score_sum = 0
        for key in ncc_score.keys():
            score_sum += ncc_score[key] * ratio_score[key]

        disco_score[model] = score_sum

    tw_sfda = w_kendall_metric(disco_score, finetune_acc, dset)
    print("Kendall  dataset:{:12s}:{:2.3f}".format(dset, tw_sfda))
